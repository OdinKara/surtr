...
export function entryFor({ item, op, targetId, runId, testMode }) {
  return {
    ...
    resolvedAt: null,
    responseStatus: null,
    // How many times this item was dispatched. More than one means it was rate
    // limited and re-dispatched after the window, not that it was acted on
    // twice - only the final attempt can have had an effect.
    attempts: 0,
...
const CSV_COLUMNS = [
  ...
  'createdAt', 'outcome', 'outcomeDetail', 'responseStatus', 'attempts', 'resolvedAt',
