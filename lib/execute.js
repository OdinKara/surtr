 ...
 * its own.
 */
export function isUnrecoverableFailure({ status, body }) {
  const errors = body && Array.isArray(body.errors) ? body.errors : [];
 ...
 * streak.
 */
export function recordOutcome(breaker, { outcome, status, body, detail, targetId, op }) {
  ...
  if (outcome !== OUTCOME.FAILED) return breaker;      // unverified, skipped: neutral
  ...
  const { unrecoverable, reason } = isUnrecoverableFailure({ status, body });
  if (unrecoverable) {
