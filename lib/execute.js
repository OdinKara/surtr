...
export const CONFIRMED_SUCCESS_SHAPES = {
  ...
  DeleteRetweet: (body, ctx) => {
    ...
    if (echoed === undefined || echoed === null) {
      return { ok: false, detail: 'data.unretweet present but no echoed rest_id to verify' };
    }
...
export function recordOutcome(
  ...
  // unverified, deferred, skipped: neutral. A deferred item was never actually
  // rejected - it was rate limited - so it must not count toward a failure
  // streak any more than the raw 429 does.
