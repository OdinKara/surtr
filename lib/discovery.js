...
/**
 * Timeline operations, in priority order.
 *
 * X SPLIT the old single profile timeline into tab-scoped operations.
 * `UserTweetsAndReplies` was absent from all 364 requests in a live capture of
 * a logged-in profile page: it is a dead name, kept last here only so that an
 * older deployment still resolves rather than failing outright.
 *
 ...
 * how `UserTweetsAndReplies` looked fine for so long. Everything below is
 * reported as "in bundle" until a request against it actually returns, and the
 * panel says which.
 */
...
export const OPERATIONS = [...REQUIRED_OPERATIONS, ...TIMELINE_CANDIDATES];
...
// in current builds. We try these first and widen only if they come up empty.
...
export async function discover({ force = false, onProgress = () => {} } = {}) {
  ...
  // A timeline candidate is USABLE if its queryId is in the bundle. The first
  // one present in priority order is selected. Every other one found is
  // reported too, so the panel can show what was on offer rather than just the
  // winner - that list is what tells you X has renamed something again.
  const timelineFound = TIMELINE_CANDIDATES.filter((op) => queryIds[op]);
  const selectedTimeline = timelineFound[0] || null;

  const missing = REQUIRED_OPERATIONS.filter((op) => !queryIds[op]);
  if (!selectedTimeline) missing.push('timeline operation');
  ...
  const record = {
    ...
    bearer,
    queryIds,
    // Which timeline operation we will actually call, and what else was there.
    selectedTimeline,
    timelineFound,
    // Set to the operation name only once a request against it has actually
    // returned. Until then the panel must not claim the site serves it -
    // being in the bundle is not the same thing, which is precisely how a dead
    // operation name went unnoticed.
    confirmedLive: null,
...
/**
 * Record that an operation actually answered. Called by the executor after the
 * first successful timeline page, and the only thing that upgrades the panel
 * from "in bundle" to "confirmed live".
 */
export async function markConfirmedLive(operationName) {
  const rec = (await store.get(store.KEY.DISCOVERY)) || null;
  if (!rec) return null;
  rec.confirmedLive = operationName;
...
export async function applyManual({ bearer, queryIds }) {
  ...
  merged.timelineFound = TIMELINE_CANDIDATES.filter((op) => merged.queryIds[op]);
  merged.selectedTimeline = merged.timelineFound[0] || null;
  merged.confirmedLive = null;
  merged.missing = (merged.bearer ? [] : ['bearer'])
    .concat(REQUIRED_OPERATIONS.filter((op) => !merged.queryIds[op]))
    .concat(merged.selectedTimeline ? [] : ['timeline operation']);
