...
/** Operations phase 1 needs. Enumeration only. */
export const OPERATIONS = ['UserByScreenName', 'UserTweetsAndReplies'];
...
export async function discover({ force = false, onProgress = () => {} } = {}) {
  ...
  const missing = OPERATIONS.filter((op) => !queryIds[op]);
  const record = {
...
export async function applyManual({ bearer, queryIds }) {
  ...
  merged.missing = (merged.bearer ? [] : ['bearer'])
    .concat(OPERATIONS.filter((op) => !merged.queryIds[op]));
