 ...
 * ceiling for this endpoint, it differs per endpoint and per account, and a
...
function absorbRateHeaders(res) {
  const num = (h) => {
...
export async function gql({
  ...
  for (let round = 0; round < MAX_NEGOTIATION_ROUNDS; round += 1) {
    ...
    if (res.status === 429) {
      ...
      onLog(
        'warn',
        'HTTP 429 rate limited. Observed limit=' + (rate.limit ?? '?') +
        ', remaining=' + (rate.remaining ?? '?') + '. Waiting ' +
        Math.round(waitMs / 1000) + 's until reset, concurrency now ' + rate.concurrency + '.'
      );
      await sleep(waitMs);
      continue;
    }
