...
export const KEY = {
  ...
  EXEC: NS + 'exec',           // PHASE 2 run state: progress, counts, outcome
  TEST_VERIFIED: NS + 'testVerified', // attestation that the 5-item test was checked
  KILLLOG_OFFERED: NS + 'killlogOffered', // run ids whose log was already offered
};
...
// PARTIAL, not ERROR: a stream failed or was skipped but results were still
// enumerated, and nothing enumerated is ever discarded. Calling that "error"
// would invite throwing away data that is perfectly good.
...
export function emptyJob() {
  return {
    ...
    excluded: 0,
    pages: 0,
    // One entry per stream (posts, reposts): status, op, pages, cursor,
    // seenCursors, endReason, ceilingSuspected, error, rate. Pagination state
    // is PER STREAM so a reload resumes mid-stream without redoing a finished
    // one. See lib/streams.js.
    streams: [],
    currentStream: null,
    // Ids that arrived from more than one stream. Should always be zero - the
    // streams are tab-scoped and disjoint - so a non-zero value is a defect
    // signal about our model of X's operations, not noise.
    crossStreamDuplicates: 0,
    crossStreamDuplicateIds: [],
    // Rate observations keyed by operation name. Never merged across
    // operations; different endpoints carry different budgets.
    rates: null,
    error: null,
  };
}
